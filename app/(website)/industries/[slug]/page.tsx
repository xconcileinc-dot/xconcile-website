    ...
    <>
      ...
      >
        <div className="absolute inset-0 opacity-10">
          <Image
            src={industry.heroBackgroundImage || industry.heroImage || 'https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=1920&h=1080&fit=crop'}
            alt={industry.heroBackgroundImageAlt || "Background"}
            fill
      ...
      <Section background="white" spacing="lg">
        ...
        {industry.subIndustries && industry.subIndustries.length > 0 && (
          <Section background="white" spacing="lg" className="bg-neutral-50">
            <Container>
        ...
        <Container>
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 md:gap-8">
            <div className="text-center md:text-left flex-1">
              <h3 className="text-2xl md:text-3xl font-bold text-white mb-3">{industry.introCTA?.title || "Ready to Get Started?"}</h3>
              <p className="text-base md:text-lg text-neutral-300">{industry.introCTA?.description || "Get a customized quote for your business needs"}</p>
            </div>
            <div className="flex-shrink-0">
              <Link href={industry.introCTA?.cta?.link || "/contact"}>
                <Button
                  variant={industry.introCTA?.cta?.variant || "secondary"}
                  size="lg"
                ...
                >
                  {industry.introCTA?.cta?.text || "Request a Quote"}
                </Button>
              </Link>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-center relative">
            ...
            <div className="relative h-[32rem] lg:h-[38rem] animate-fade-in-right lg:order-2 lg:col-span-5">
              ...
              <div className="relative h-full rounded-[60px] overflow-hidden shadow-2xl border-8 border-white/5">
                <Image
                  src={industry.whyPartnerImage || "https://images.unsplash.com/photo-1552664730-d307ca884978?w=800&h=1200&fit=crop"}
                  alt={industry.whyPartnerImageAlt || "Client success story"}
                  fill
                  className="object-cover"
                />
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-primary-900 via-primary-900/40 to-transparent p-12">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <span className="text-secondary-400 font-bold uppercase tracking-widest text-xs">
                      {industry.whyPartnerImageLabel || "Innovation Partner"}
                    </span>
                  </div>
                  <p className="text-2xl text-white font-bold leading-tight">
                    {industry.whyPartnerImageTagline || "Driving performance through strategic financial leadership."}
                  </p>
                </div>
              </div>
            </div>
          ...
          <Container>
            <div className="text-center mb-12 animate-fade-in-up">
              <h2 className="text-3xl md:text-4xl font-bold text-neutral-900 mb-4">
                {industry.testimonialsTitle || "Industry Success Stories"}
              </h2>
              <p className="text-lg text-neutral-600 max-w-2xl mx-auto">
                {industry.testimonialsDescription || `Trusted by leading ${industry.title} businesses worldwide`}
              </p>
            </div>
            <TestimonialSlider
        ...
        <Container className="relative z-10">
          <div className="max-w-4xl mx-auto">
            <h2 className="text-3xl md:text-5xl font-bold mb-8 text-white leading-[1.1] tracking-tight">
              {industry.finalCTA?.title || `Ready to Upgrade Your ${industry.title}?`}
            </h2>
            <p className="text-lg md:text-xl mb-10 text-white/95 leading-relaxed font-medium">
              {industry.finalCTA?.description || "Join elite businesses that trust our experts with their growth. Schedule your strategy session today."}
            </p>
            <div className="flex justify-center">
              <Link href={industry.finalCTA?.cta?.link || "/contact"}>
                <Button
                  variant={industry.finalCTA?.cta?.variant || "secondary"}
                  size="lg"
                ...
                >
                  {industry.finalCTA?.cta?.text || "Get Started Today"}
                </Button>
              </Link>
            </div>
          </div>
